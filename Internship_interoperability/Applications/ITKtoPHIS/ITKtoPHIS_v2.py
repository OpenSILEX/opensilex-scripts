...
class MainWindow(QDialog):
    ...
    def getSetValuesFile (self):
        self.setting_variables = QSettings('ITKtoPHIS', 'Filename')
    ...
    def reussi(self, mess):
        QMessageBox.information(self,"ITKtoPHIS - Succès !",mess)

    def evt_update_progress(self, val):
        self.progressBar.setValue(val)

    def error(self,mess):
        QMessageBox.critical(self,"ITKtoPHIS - Erreur !",mess)
...
class WorkerThread(QThread):
    ...
    def run(self):
        try :
            ...
            filepath = MainWindow().get_path_value()
            ...
            # Fonction qui permet de déclarer dans PHIS les évènements qui ne sont pas déjà déclarés: on obtient en sortie une liste d'URI d'OS dont l'évènement est déjà déclaré, le nombre d'interventions déclarées et une liste d'URI d'OS dont l'évènement est pas déjà déclaré
            def sendEventCheckExist(bodys,pythonClient):
                ...
                for body in bodys:                 
                    ...
                    if exist:
                        ...
                        sent+=1
                        decla.append(body.targets[0])
                return errors,decla, sent

            # On exécute la focntion sendEventCheckExist
            logs, decla, nsend = sendEventCheckExist(bodys,pythonClient)

            # On construit le texte contenant le détail de l'exécution
            uri_count_deja=""
            uri_count_pas_deja = ""
            concerne="Concerne le(s) OS :"
            if len(logs)>0 and len(decla)>0: # si on a des évènements déjà déclarés et des évènements pas déjà déclarés
                ...
                uri_count_deja=df_err["uri"].value_counts()
                
                # on convertit en dataframe la liste des URI d'OS dont l'évènement est pas déjà déclaré
                df_decla = pd.DataFrame({"uri":decla})
                # on récupère combien de fois on retrouve chaque URI = nbre d'évènement pas déjà déclaré par OS
                uri_count_pas_deja = df_decla["uri"].value_counts()
                

                txt_detail = "L'opération s'est déroulée avec succès !\n\nNombre d'interventions déclarées : {0}\n{2}\n{4}\nNombre d'interventions déjà déclarées : {1} \n{2} \n{3}".format(nsend,len(logs),concerne,uri_count_deja,uri_count_pas_deja).replace("Name: uri, dtype: int64","")
            
            elif len(logs)>0 : # si on a que des évènements déjà déclarés
                # on convertit en dataframe la liste des URI d'OS dont l'évènement est déjà déclaré 
                df_err=pd.DataFrame({"uri":logs})
                # on récupère combien de fois on retrouve chaque URI = nbre d'évènement déjà déclaré par OS
                uri_count_deja=df_err["uri"].value_counts()            

                txt_detail = "L'opération s'est déroulée avec succès !\n\nNombre d'interventions déclarées : {0}\n\nNombre d'interventions déjà déclarées : {1} \n{2} \n{3}".format(nsend,len(logs),concerne,uri_count_deja).replace("Name: uri, dtype: int64","")

            elif len(decla)>0 :  # si on a que des évènements pas déjà déclarés
                # on convertit en dataframe la liste des URI d'OS dont l'évènement est pas déjà déclaré
                df_decla = pd.DataFrame({"uri":decla})
                # on récupère combien de fois on retrouve chaque URI = nbre d'évènement pas déjà déclaré par OS
                uri_count_pas_deja = df_decla["uri"].value_counts()
            
                txt_detail = "L'opération s'est déroulée avec succès !\n\nNombre d'interventions déclarées : {0}\n{2}\n{3}\nNombre d'interventions déjà déclarées : {1}".format(nsend,len(logs),concerne,uri_count_pas_deja).replace("Name: uri, dtype: int64","")

            else : # si aucun évènement
                txt_detail = "L'opération s'est déroulée avec succès !\n\nNombre d'interventions déclarées : {0}\n\nNombre d'interventions déjà déclarées : {1}".format(nsend,len(logs)).replace("Name: uri, dtype: int64","")

            # Affiche le pop-up pour indiquer le résultat de l'exécution
            self.succes_mess.emit(txt_detail)
